          ...
          <div className="space-y-3">
            {clients.slice(0, 5).map(client => (
              <div key={client.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                <div>
                  <div className="font-medium text-slate-800">{client.short_name}</div>
          ...
          <div className="space-y-3">
            {contracts.slice(0, 5).map(contract => (
              <div key={contract.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                <div>
                  <div className="font-medium text-slate-800">{contract.contract_number}</div>
                  <div className="text-xs text-slate-500">Expires: {contract.end_date}</div>
                </div>
                <span className={`text-xs px-2 py-1 rounded ${contract.its_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
